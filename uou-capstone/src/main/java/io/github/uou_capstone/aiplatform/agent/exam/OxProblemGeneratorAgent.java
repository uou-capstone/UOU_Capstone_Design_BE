...
/**
 * OX Problem Generator Agent
 * OX 문제 생성 Agent
 */
@Slf4j
public class OxProblemGeneratorAgent extends AbstractAgent {

    public OxProblemGeneratorAgent(WebClient aiServiceWebClient, ObjectMapper objectMapper, AgentPerformanceLogger performanceLogger) {
        super(aiServiceWebClient, objectMapper, performanceLogger, "OxProblemGeneratorAgent");
    }

    @Override
    protected String getEndpoint() {
        return "/api/test-gen/ox-problem";
    }
     ...
     * @return 생성된 OX 문제 리스트
     */
    public List<OxProblemDto> generateOxProblems(String lectureContent, TestProfileDto profile, Integer targetCount) {
        Map<String, Object> context = new HashMap<>();
        context.put("lecture_content", lectureContent);
        context.put("profile", profile);
        context.put("target_count", targetCount);

        AgentRequest request = new SimpleAgentRequest("Generate OX problems", context);
