...
학생 입장 흐름이 단일 코드 즉시 등록에서 **교사 승인형**으로 전환됐다. 자세한 가이드는 `docs/handoff/COURSE_JOIN_REQUEST_FE.md` (가입 요청 본체) + `docs/handoff/NOTIFICATION_FE.md` (학생 상태 조회 + 알림 인프라) 참조.
...
학생/교사 공용. 진실 원천은 DB 저장 알림이고, SSE 스트림은 실시간 편의용이다(연결이 끊겨도 GET 목록으로 복구 가능). 자세한 가이드 / 코드 예제 / 화면 흐름: `docs/handoff/NOTIFICATION_FE.md`.
